...
public class Person {
  ...
  public static void main(String[] args) {
    ...
    Person thePerson = new Person("Alex", 24); 
    // Create another instance of Person with a different name and age and
    // assign it to a different variable
    Person anotherPerson = new Person("Marvin", 21);
    // Print the first person
    System.out.println(thePerson.toString());
    // Print the second person
    System.out.println(anotherPerson.toString());
    // Get the name of the first person and store it in a local variable
    String aName = thePerson.name;
    System.out.println(aName);
    // Using the birthYear method, get the birth year of the first person
    // and store it in a local variable. Input the actual current year (e.g. 2025)
    // as the argument.
    int firstBYear = thePerson.birthYear(2025);
    // In a separate statement, print the local variable holding the birth year.
    System.out.println(firstBYear);
    /**
     * Terminology!
     * 
